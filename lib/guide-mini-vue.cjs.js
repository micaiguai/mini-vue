...
function createRenderer(options) {
    ...
    function patchKeyedChildren(c1, c2, container, parentComponent, anchor) {
        ...
        if (i > e1) {
            ...
            const toBePatchedCount = e2 - i + 1;
            let maxNewIndex = 0;
            let moved = false;
            const newIndexToOldIndexMap = new Array(toBePatchedCount).fill(0);
            let patchCount = 0;
            ...
            const increaseSequence = moved ? getSequence(newIndexToOldIndexMap) : [];
            let j = increaseSequence.length - 1;
            for (let index = toBePatchedCount - 1; index >= 0; index--) {
                const newIndex = index + i;
                const newVnode = c2[newIndex];
                const anchor = newIndex + 1 > l2 - 1 ? null : c2[newIndex + 1].el;
                if (newIndexToOldIndexMap[index] === 0) {
                    patch(null, newVnode, container, parentComponent, anchor);
                }
                else if (moved) {
                    if (j < 0 || index !== increaseSequence[j]) {
